...
class Portal:
    ...
    def __init__(self, name: str, topic: str, bootstrap_servers: str, config: PortalConfig | None = None, ) -> None:
    ...
    def _initialize_consumer_group(self) -> str:
        ...
        return self._config.consumer_group or default_consumer_group

    def _initialize_teleporter(self) -> Teleporter:

        kafka_producer_config: KafkaProducerConfig = KafkaProducerConfig(client_id=self._name,
        ...
        return Teleporter(kafka_producer_config=kafka_producer_config,
                          max_queue_size=self._config.max_queue_size,
                          circuit_breaker_config=circuit_breaker_config)

    def __str__(self) -> str:
        return (f"Portal(name={self._name}, topic={self._topic}, "
    ...
    def teleportable(
        ...
        # TODO: Validate all parameters, create a validation function
        if retry < 1:
        ...
        def decorator(func: F) -> Worklet[F] | None:
            ...
            if self._worklet_registry.portal is None:
                portal_model = PortalModel(name=self._name,
                                           topic=self._topic,
                                           bootstrap_servers=self._bootstrap_servers,
                                           consumer_group=self._consumer_group, )
                self._worklet_registry.set_portal(portal_model)

            # Register portal & worklet
            id: str = name or func_utils.get_canonical_func_name(func=func)
            task_retry: TaskRetry = TaskRetry(retry=retry,
            ...
            worklet: WorkletModel = WorkletModel(id=id,
                                                 portal=self._name,
                                                 func=func,
                                                 retry=task_retry, )
